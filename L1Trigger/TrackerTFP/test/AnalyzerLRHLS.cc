//
// Created by Maziar Ghorbani on 08/07/2020.
//
...
namespace trackerTFP {
  ...
  /*! \class  trackerTFP::AnalyzerLRHLS
   *  \brief  Class to analyze hardware like structured TTStub Collection generated by Linear Regression
   *  \author Thomas Schuh, Modified by Maziar Ghorbani
   *  \date   2020, June
   */
  class AnalyzerLRHLS : public one::EDAnalyzer<one::WatchRuns, one::SharedResources> {
  public:
    AnalyzerLRHLS(const ParameterSet& iConfig);
  ...
  AnalyzerLRHLS::AnalyzerLRHLS(const ParameterSet& iConfig) : useMCTruth_(iConfig.getParameter<bool>("UseMCTruth")), nEvents_(0) {
    usesResource("TFileService");
    // book in- and output ED products
    const string& label = iConfig.getParameter<string>("LabelLRHLS");
  ...
  void AnalyzerLRHLS::beginRun(const Run& iEvent, const EventSetup& iSetup) {
  ...
  void AnalyzerLRHLS::analyze(const Event& iEvent, const EventSetup& iSetup) {
    // read in ht products
  ...
  void AnalyzerLRHLS::endJob() {
    // printout LRHLS summary
  ...
  //
  void AnalyzerLRHLS::formTracks(const TTDTC::Stream& stream, vector<vector<TTStubRef>>& tracks) const {
  ...
  //
  void AnalyzerLRHLS::associate(const vector<vector<TTStubRef>>& tracks, const StubAssociation* ass, set<TPPtr>& tps, int& sum) const {
...
DEFINE_FWK_MODULE(trackerTFP::AnalyzerLRHLS);